...
class StatisticsFragment : Fragment() {
    ...
    private fun setUpData(){
        ...
        binding.tvAvgSpeedValueR.text= String.format("%.2f",viewModel.getAverageSpeed(ActivityType.RUN))
        binding.tvTimeValueR.text= formatDuration(viewModel.getTotalTime(ActivityType.RUN))
        binding.tvDistanceValueR.text=String.format("%.2f",viewModel.getTotalDistance(ActivityType.RUN))

        binding.tvCaloriesValueC.text=viewModel.getTotalCalories(ActivityType.CYCLE).toString()
        binding.tvAvgSpeedValueC.text= String.format("%.2f",viewModel.getAverageSpeed(ActivityType.CYCLE))
        binding.tvTimeValueC.text= formatDuration(viewModel.getTotalTime(ActivityType.CYCLE))
        binding.tvDistanceValueC.text=String.format("%.2f",viewModel.getTotalDistance(ActivityType.CYCLE))

        binding.tvCaloriesValueW.text=viewModel.getTotalCalories(ActivityType.WALK).toString()
        binding.tvAvgSpeedValueW.text= String.format("%.2f",viewModel.getAverageSpeed(ActivityType.WALK))
        binding.tvTimeValueW.text= formatDuration(viewModel.getTotalTime(ActivityType.WALK))
        binding.tvDistanceValueW.text=String.format("%.2f",viewModel.getTotalDistance(ActivityType.WALK))
    }
}